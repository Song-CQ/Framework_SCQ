    ...
    public class PETimer : IDisposable
    {
        ...
        public PETimer(int interval = 0)
        {
            ...
            if (interval != 0)
            {
                ...
                srvTimer.Elapsed += (object sender, ElapsedEventArgs e) => { Update(); };
                srvTimer.Start();
            }
        ...
        public void Dispose()
        {
            ...
            tmpTimeTasksLst = null;
            TimeTasksLst = null;

            tmpFrameTasksLst = null;
            FrameTasksLst = null;
            srvTimer = null;
        }
        ...
        /// <param name="timeType">回调间隔的时间单位，默认为毫秒</param>
        /// <returns> 是否成功替换</returns>
        public bool ReplaceTimeTask(int tid, Action<int> _cb, double _delsy, int _count = 1,
            TimeType timeType = TimeType.Millisecond)
        {
            bool isRep = false;
            switch (timeType) //转换单位为毫秒
            {
        ...
        /// <param name="timeType">回调间隔的时间单位，默认为毫秒</param>
        /// <returns></returns>
        public int AddTimeTask(Action<int> _cb, double _delsy, int _count = 1,
            TimeType timeType = TimeType.Millisecond)
        {
            switch (timeType) //转换单位为毫秒
            {
